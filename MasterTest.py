...
class UncannyValleyApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Uncanny Valley Experiment -- TEST / DEV BUILD (not for real participants)")
    ...
    def show_welcome_screen(self):
        ...
        test_banner = tk.Label(self.container, text="TEST / DEV BUILD -- NOT FOR REAL PARTICIPANTS", font=("Arial", 13, "bold"), fg="#ffffff", bg="#c0392b", padx=10, pady=4)
        test_banner.pack(pady=(8, 32))
        ...
        self.start_btn = tk.Button(
            self.container, 
            text="Start Experiment (No Sensor Connected -- Test Mode)", 
            font=("Arial", 14, "bold"), 
            bg="#2ecc71", 
            fg="black", 
            padx=25, 
    ...
    def connection_success_ui(self):
        ...
        self.start_btn.config(text="Start Experiment")
    ...
    def begin_experiment_session(self):
        if not self.sensor_connected:
            proceed = messagebox.askyesno(
                "No Sensor Connected",
                "No Polar sensor is connected. If you continue, NO physiological (PPI) "
                "data will be recorded for this session -- and this cannot be recovered "
                "afterward.\n\n"
                "Continue anyway in test mode (e.g. to test video/camera/UI only)?\n\n"
                "Choose 'No' to go back and click '1. Connect Bluetooth Sensor' first."
            )
            if not proceed:
                return
